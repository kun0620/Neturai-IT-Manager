...
export default function StockControlPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <StockControlKpiSection
        totalOnHand={totalOnHand}
        totalReserved={totalReserved}
        totalAvailable={totalAvailable}
      />

      <StockCreateItemSection
        canManage={canManage}
        pending={createStockItem.isPending}
        newSku={newSku}
        setNewSku={setNewSku}
        newName={newName}
        ...
        setNewReorderPoint={setNewReorderPoint}
        newReorderQty={newReorderQty}
        setNewReorderQty={setNewReorderQty}
        locationKey={locationKey}
        setLocationKey={setLocationKey}
        newOpeningQty={newOpeningQty}
        setNewOpeningQty={setNewOpeningQty}
        handleCreateItem={handleCreateItem}
      />

      <StockControlContentSection
        ...
        setReceiveSerial={setReceiveSerial}
        issueQty={issueQty}
      ...
      <StockMovementSection
        ...
        movements={movements}
        openRequestFromMovement={openRequestFromMovement}
      />
...
type StockControlActionBarProps = {
  search: string;
...
function StockControlActionBar({
  search,
  setSearch,
  stockItems,
  itemId,
    ...
    <section className="rounded-xl border bg-card p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="w-full sm:max-w-md">
            <Select value={itemId} onValueChange={setItemId}>
              <SelectTrigger>
                <SelectValue placeholder="Select item for operations" />
              </SelectTrigger>
              <SelectContent>
                {stockItems.map((item) => (
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant={lowStockCount > 0 ? 'destructive' : 'outline'} className="font-medium">
...
type StockControlKpiSectionProps = {
  totalOnHand: number;
  totalReserved: number;
  totalAvailable: number;
};

function StockControlKpiSection({
  totalOnHand,
  totalReserved,
  totalAvailable,
}: StockControlKpiSectionProps) {
  return (
...
type StockCreateItemSectionProps = {
  canManage: boolean;
  pending: boolean;
  newSku: string;
  setNewSku: (value: string) => void;
  newName: string;
  setNewName: (value: string) => void;
  newCategory: string;
  setNewCategory: (value: string) => void;
  newMode: StockTrackingMode;
  setNewMode: (value: StockTrackingMode) => void;
  newReorderPoint: string;
  setNewReorderPoint: (value: string) => void;
  newReorderQty: string;
  setNewReorderQty: (value: string) => void;
  locationKey: string;
  setLocationKey: (value: string) => void;
  newOpeningQty: string;
  setNewOpeningQty: (value: string) => void;
  handleCreateItem: () => void;
};

function StockCreateItemSection({
  canManage,
  pending,
  newSku,
  setNewSku,
  ...
  setNewOpeningQty,
  handleCreateItem,
}: StockCreateItemSectionProps) {
  return (
    <section className="rounded-xl border bg-card p-4 space-y-4">
      <div>
        <h2 className="text-base font-semibold">Create stock item</h2>
        <p className="text-xs text-muted-foreground">Define a new SKU and starting balance.</p>
      </div>
      ...
      <Button onClick={handleCreateItem} disabled={!canManage || pending}>
...
type StockControlContentSectionProps = {
  filteredItems: StockItemView[];
  selectedItem: StockItemView | null;
  canManage: boolean;
  stockItems: StockItemView[];
  itemId: string;
  setItemId: (value: string) => void;
  ...
  receiveQty: string;
  setReceiveQty: (value: string) => void;
  receiveSerial: string;
  setReceiveSerial: (value: string) => void;
  issueQty: string;
  setIssueQty: (value: string) => void;
  selectedUnitId: string;
  setSelectedUnitId: (value: string) => void;
  inStockUnits: StockUnitRow[];
  adjustDelta: string;
  setAdjustDelta: (value: string) => void;
  handleReceive: () => void;
  handleIssue: () => void;
  handleAdjust: () => void;
  lowStockItems: StockItemView[];
};

function StockControlContentSection(props: StockControlContentSectionProps) {
  return (
    <section className="flex flex-col gap-6 xl:flex-row">
      <StockInventoryTableSection filteredItems={props.filteredItems} />

      <aside id="stock-operations" className="w-full space-y-4 xl:w-[340px]">
        <StockOperationsSection
          ...
          setLocationKey={props.setLocationKey}
          note={props.note}
          setNote={props.setNote}
          receiveQty={props.receiveQty}
          setReceiveQty={props.setReceiveQty}
          receiveSerial={props.receiveSerial}
          setReceiveSerial={props.setReceiveSerial}
          issueQty={props.issueQty}
          setIssueQty={props.setIssueQty}
          selectedUnitId={props.selectedUnitId}
          setSelectedUnitId={props.setSelectedUnitId}
          inStockUnits={props.inStockUnits}
          adjustDelta={props.adjustDelta}
          setAdjustDelta={props.setAdjustDelta}
          handleReceive={props.handleReceive}
          handleIssue={props.handleIssue}
          handleAdjust={props.handleAdjust}
        />
        <StockAlertsSection lowStockItems={props.lowStockItems} />
      </aside>
    </section>
  );
}

function StockInventoryTableSection({ filteredItems }: { filteredItems: StockItemView[] }) {
  return (
              ...
              return (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">
                    <span className={lowStock ? 'font-semibold text-red-600' : undefined}>{item.available}</span>
                  </TableCell>
                  <TableCell className="text-right">
                    {item.reorder_point}
                    {lowStock && (
...
type StockOperationsSectionProps = {
  selectedItem: StockItemView | null;
  canManage: boolean;
  stockItems: StockItemView[];
  itemId: string;
...
function StockAlertsSection({ lowStockItems }: { lowStockItems: StockItemView[] }) {
  return (
    <section className="rounded-xl border bg-card p-4">
...
type StockMovementSectionProps = {
  ...
  openRequestFromMovement: (referenceType: string | null, referenceId: string | null) => void;
};

function StockMovementSection({
  movementLoading,
  movements,
  openRequestFromMovement,
}: StockMovementSectionProps) {
  return (
    <section className="rounded-xl border bg-card">
      <div className="border-b p-4">
            ...
            <TableBody>
              {movements.map((movement) => (
                <TableRow key={movement.id}>
                  ...
                  <TableCell>{movement.location_from ?? '-'}</TableCell>
                  <TableCell>{movement.location_to ?? '-'}</TableCell>
                  <TableCell>
                    {movement.reference_id ? (
                      <button
                        type="button"
                        className="text-primary underline-offset-4 hover:underline"
                        onClick={() => openRequestFromMovement(movement.reference_type, movement.reference_id)}
                      >
