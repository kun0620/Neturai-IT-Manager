/*
  # Fix update_ticket_with_logs priority type mismatch

  - Avoid CASE type mismatch between text and ticket_priority_enum
  - Keep status/assignee/due-date audit logs
  - Support environments where tickets.priority is either enum or text
*/
...
BEGIN
  ...
  IF p_updates ? 'priority' THEN
    ...
    IF v_priority_udt = 'ticket_priority_enum' THEN
      EXECUTE
        'UPDATE public.tickets
         SET priority = ($1->>''priority'')::public.ticket_priority_enum
         WHERE id = $2
         RETURNING *'
      INTO updated_row
...
GRANT EXECUTE ON FUNCTION public.update_ticket_with_logs(uuid, jsonb, uuid) TO service_role;
