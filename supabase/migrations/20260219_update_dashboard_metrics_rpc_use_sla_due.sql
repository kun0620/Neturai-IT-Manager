/*
  # Update dashboard metrics RPC to use SLA due time

  - Keep function signature unchanged
  - Compute SLA due from ticket due_at OR business-hours policy fallback
  - Use SLA due for overdue counts and trends
*/
...
AS $$
  ...
  sla_policy_map AS (
    SELECT
      lower(sp.priority::text) AS priority_key,
      sp.resolution_time_hours
    FROM public.sla_policies sp
  ),
  ticket_with_sla_due AS (
    SELECT
      ts.*,
      COALESCE(
        ts.due_at,
        CASE
          WHEN ts.created_at IS NOT NULL
            AND sp.resolution_time_hours IS NOT NULL
            AND sp.resolution_time_hours > 0
          THEN public.add_business_hours(ts.created_at, sp.resolution_time_hours)
          ELSE NULL
        END
      ) AS sla_due_at
    FROM ticket_scope ts
    LEFT JOIN sla_policy_map sp
      ON lower(ts.priority::text) = sp.priority_key
  ),
  closed_events AS (
    SELECT
  ...
  SELECT
    ...
    (
      SELECT count(*)
      FROM ticket_with_sla_due t, bounds b
      WHERE t.sla_due_at < b.now_ts
        AND t.status <> 'closed'
    ...
    (
      SELECT
        count(*) FILTER (
          WHERE t.sla_due_at >= b.start_today
            AND t.sla_due_at < b.now_ts
            AND t.status <> 'closed'
        )
        - count(*) FILTER (
            WHERE t.sla_due_at >= b.start_yesterday
              AND t.sla_due_at < b.start_today
              AND t.status <> 'closed'
          )
      FROM ticket_with_sla_due t, bounds b
