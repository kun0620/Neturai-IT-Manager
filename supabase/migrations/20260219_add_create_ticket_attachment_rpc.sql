/*
  # Add secure RPC for ticket attachment metadata insert

  - Enforces ticket access
  - Enforces attachment size/type policy from settings
  - Inserts attachment row as auth user
*/

DROP FUNCTION IF EXISTS public.create_ticket_attachment(uuid, text, text, text, bigint);

CREATE FUNCTION public.create_ticket_attachment(
  p_ticket_id uuid,
  p_file_name text,
  p_storage_path text,
...
BEGIN
  ...
  IF coalesce(p_size_bytes, 0) < 0
    OR coalesce(p_size_bytes, 0) > (greatest(v_max_mb, 1) * 1024 * 1024) THEN
...
GRANT EXECUTE ON FUNCTION public.create_ticket_attachment(uuid, text, text, text, bigint) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_ticket_attachment(uuid, text, text, text, bigint) TO service_role;
